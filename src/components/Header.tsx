    ...
    <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--bg)]/90 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          <Link href="/courses" className="btn-ghost">Курсы</Link>
          <Link href="/leaderboard" className="btn-ghost">Лидерборд</Link>
          {user || isInProfile ? (
            <>
              ...
              <div className="relative">
                <button onClick={() => setMenuOpen(!menuOpen)} className="btn-ghost flex items-center gap-2 rounded-full">
                  {user?.avatar ? (
                ...
                {menuOpen && (
                  <>
                    ...
                    <div className="absolute right-0 top-full z-20 mt-1 w-48 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] py-1 shadow-xl">
                      <Link href="/profile" className="block px-4 py-2 text-sm hover:bg-slate-100" onClick={() => setMenuOpen(false)}>Профиль</Link>
                      <button onClick={logout} className="w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-slate-100">Выйти</button>
                    </div>
                  </>
                )}
              </div>
            ...
            <>
              <Link href="/login" className="btn-ghost">Вход</Link>
        ...
        <div className="flex items-center gap-2 md:hidden">
          {(user || isInProfile) && (
            <Link href="/profile" className="rounded-full bg-violet-100 px-2 py-1 text-xs font-bold text-violet-700">
          ...
          {user || isInProfile ? (
            <button onClick={logout} className="btn-ghost text-sm text-red-400">Выйти</button>
          ) : (
            <>
              <Link href="/login" className="btn-ghost text-sm">Вход</Link>
              <Link href="/register" className="btn-primary text-sm">Регистрация</Link>
            </>
          )}
        </div>
      </div>
